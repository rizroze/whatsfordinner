# Directory & Launch Submissions

Status key: ✅ done · ⏳ pending

---

## ✅ Product Hunt — done

**Tagline (60 chars):** AI meal plans with recipes and a grocery list in 30 seconds

**Description:**
What's For Dinner generates personalized weekly meal plans with full recipes and a consolidated grocery list. Tell it your dietary preferences, allergies, household size, and budget — get a complete week of meals in 30 seconds.

Free personalized preview, no account required. Subscribers get a fresh 7-day plan delivered to their inbox every Sunday.

Supports keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, and more. Portions auto-adjust to your household size.

**Maker Comment:**
I built this because my family had the same conversation every night: "what's for dinner?" I wanted something that just handles it — not a recipe app where you still have to pick everything yourself, but a planner that knows what you like and gives you the whole week with a grocery list you can actually shop from.

**Topics:** Meal Planning, AI, Food & Drink, Productivity

---

## ✅ AlternativeTo — done

**URL:** https://whatsfordinner.fit
**Category:** Meal Planning
**Alternative To:** Mealime, Eat This Much, PlateJoy, Plan to Eat
**Tags:** Meal Planning, Grocery List, AI, Recipes, Weekly Planner
**Description:** AI-powered meal planner that generates personalized weekly meal plans with full recipes and a consolidated grocery list. Supports all dietary needs. Free 3-day plan available.
**Commercial:** Freemium ($7.99/mo or $59.99/yr)
**Platform:** Web

---

## ⏳ SaaSHub — saashub.com/submit

**Product Name:** What's For Dinner
**Website:** https://whatsfordinner.fit
**Category:** Meal Planning Software
**Tagline:** AI meal planner with weekly plans, recipes & grocery list
**Description:**
What's For Dinner is an AI-powered meal planner that creates personalized weekly meal plans with full recipes and a consolidated grocery list. Enter your dietary preferences, household size, allergies, and budget — get a complete week of meals in seconds.

Free tier includes a 3-day plan with no credit card required. Paid subscribers receive a fresh 7-day plan every Sunday delivered to their inbox.

Supports 20+ dietary preferences including keto, vegan, vegetarian, gluten-free, halal, dairy-free, and paleo. Portions auto-scale to household size.

**Pricing:** Freemium — Free 3-day plan · $7.99/mo · $59.99/yr
**Alternatives:** Mealime, Eat This Much, PlateJoy, Plan to Eat

---

## ⏳ There's An App For That — thereisanappforthat.com/submit

**App Name:** What's For Dinner
**URL:** https://whatsfordinner.fit
**Category:** Food & Drink / Productivity
**Short description (tweet-length):**
AI meal planner that builds your full week of meals + recipes + grocery list in 30 seconds. Free 3-day plan, no card needed.

**Long description:**
Stop answering "what's for dinner?" every night. What's For Dinner generates a complete personalized weekly meal plan — with recipes and a ready-to-shop grocery list — in under a minute.

Tell it your diet, allergies, household size, and budget. Get a week of meals you'll actually want to eat, with every recipe and one combined grocery list. Subscribers get a fresh plan every Sunday by email.

Supports keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, Mediterranean, and 20+ other dietary preferences.

---

## ⏳ SaaSWorthy — saasworthy.com (claim your listing)

Search for "What's For Dinner" first — they may auto-create a listing. Claim it, then add:

**Tagline:** AI meal planner: weekly meal plans, recipes & grocery list
**Description:**
What's For Dinner uses AI to generate personalized weekly meal plans complete with full recipes and a consolidated grocery list. It adapts to your dietary preferences, household size, allergies, and budget.

Key features:
- AI-generated 7-day meal plans tailored to your preferences
- Full recipes with ingredients, steps, and nutrition info
- One consolidated grocery list for the whole week
- 20+ dietary modes: keto, vegan, gluten-free, halal, paleo, and more
- Fresh plan delivered every Sunday (paid tier)
- Free 3-day plan, no credit card required

**Pricing:** Free · $7.99/month · $59.99/year
**Category:** Meal Planning, AI Tools

---

## ⏳ G2 — g2.com (claim listing + get reviews)

1. Go to g2.com/products/new and submit your product, OR search "What's For Dinner" and claim if auto-created
2. Fill out the profile, then ask your first users for a review — G2 reviews are the main value here

**Description for G2:**
What's For Dinner is an AI-powered meal planning tool that generates personalized weekly meal plans with full recipes and a consolidated grocery list. Users enter their dietary preferences, allergies, budget, and household size, and the AI produces a complete week of meals in seconds. A fresh plan is delivered every Sunday for subscribers. Supports keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, and 20+ other dietary styles. Free 3-day trial available with no credit card.

**Category:** Meal Planning Software
**Deployment:** Cloud, SaaS, Web-Based

---

## ⏳ Capterra / GetApp — capterra.com/submit (same parent company, one submission)

Note: Capterra/GetApp/Software Advice are all Gartner products — submitting to Capterra covers all three.

**Product Name:** What's For Dinner
**Website:** https://whatsfordinner.fit
**Category:** Meal Planning / Food Service Management
**Short description:**
AI meal planner that generates personalized weekly plans with full recipes and a grocery list. Free trial. Supports 20+ dietary preferences.

**Full description:**
What's For Dinner is an AI-powered meal planner designed for busy households. Users input their dietary preferences, allergies, household size, and budget, and the tool generates a complete personalized weekly meal plan — including full recipes with ingredients and cooking steps, and a single consolidated grocery list.

Subscribers receive a fresh 7-day meal plan by email every Sunday. The free tier offers a full 3-day plan with no credit card required.

Key capabilities:
- Personalized AI meal plan generation (3-day free / 7-day paid)
- Full recipe library with ingredients, instructions, and nutrition data
- Automatic grocery list consolidation
- Dietary support: keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, Mediterranean, and more
- Household-size portion scaling
- Weekly email delivery (paid tier)

**Pricing Model:** Freemium
**Starting Price:** $7.99/month or $59.99/year
**Free Trial:** Yes (3-day plan, no card)

---

## ⏳ ToolPilot.ai — toolpilot.ai/submit-tool

**Tool Name:** What's For Dinner
**URL:** https://whatsfordinner.fit
**Category:** AI Tools → Food & Cooking / Productivity
**Tagline:** AI meal planner with weekly plans, recipes & grocery list — free to try
**Description:**
What's For Dinner is an AI meal planning tool that generates personalized weekly meal plans complete with full recipes and a consolidated grocery list. Enter your dietary preferences, household size, allergies, and budget — get a complete week of meals in under 30 seconds.

Supports keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, Mediterranean, and 20+ dietary styles. Paid subscribers receive a fresh plan by email every Sunday. Free 3-day plan available, no credit card required.

**Pricing:** Freemium · Free · $7.99/mo · $59.99/yr
**Tags:** meal planning, AI, recipes, grocery list, food, weekly planner

---

## ⏳ Future Tools — futuretools.io/submit

**Tool Name:** What's For Dinner
**URL:** https://whatsfordinner.fit
**Category:** Life Assistant / Productivity
**Pricing:** Freemium
**Short description (used as tagline):**
AI meal planner: tells you what to cook all week, gives you the recipes, and builds your grocery list.

**Description:**
What's For Dinner uses Claude AI to generate personalized weekly meal plans with full recipes and a ready-to-shop grocery list. Answer a few questions about your diet, household, allergies, and budget — get a complete week of meals in 30 seconds.

Supports keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, Mediterranean, and 20+ other dietary modes. Free 3-day plan available. Subscribers get a fresh 7-day plan delivered every Sunday by email.

**Tags:** meal planning, food, recipes, grocery list, AI assistant, productivity

---

## ⏳ AI Tool Directory — aitoolsdirectory.com/submit

**Tool Name:** What's For Dinner
**Website:** https://whatsfordinner.fit
**Category:** Productivity / Lifestyle
**Pricing:** Freemium
**Tagline:** AI meal planner with weekly plans, full recipes & grocery list
**Description:**
What's For Dinner is an AI-powered meal planner that generates personalized weekly meal plans with full recipes and a consolidated grocery list. It adapts to your dietary preferences, household size, allergies, and budget.

Free tier: full 3-day meal plan, no sign-up required.
Paid tier ($7.99/mo): 7-day plan delivered fresh every Sunday by email.

Supports 20+ dietary preferences: keto, vegan, vegetarian, gluten-free, halal, dairy-free, paleo, Mediterranean, and more.

---

## ⏳ Ben's Bites — news.bensbites.co/posts/create

Ben's Bites is a popular AI newsletter. Submit your tool as a "post" for community listing.

**Title:** What's For Dinner — AI meal planner with weekly plans, recipes & grocery list

**Body:**
What's For Dinner generates personalized weekly meal plans complete with full recipes and a grocery list, in under 30 seconds.

You tell it:
- Your dietary preferences (keto, vegan, gluten-free, etc.)
- Household size and any allergies
- Budget constraints

It gives you:
- A full week of meals (or 3 days free)
- Every recipe with ingredients and steps
- One consolidated grocery list

Subscribers get a fresh plan emailed every Sunday. Free 3-day plan available with no credit card.

**Link:** https://whatsfordinner.fit

---

## ⏳ Hacker News — Show HN

Go to: https://news.ycombinator.com/submit

**Title:**
Show HN: What's For Dinner – AI that plans your whole week of meals, recipes, and grocery list

**URL:** https://whatsfordinner.fit

**Text (paste in the "text" field for Show HN posts):**
I built this because my family had the same "what's for dinner?" conversation every night. Recipe apps have thousands of options — that's the problem, not the solution. You still have to pick something, check the pantry, figure out portions, and build a grocery list yourself.

What's For Dinner is different: you tell it your dietary preferences, household size, allergies, and budget, and it generates a complete week of meals — with full recipes and one consolidated grocery list — in about 30 seconds.

Free tier: a full 3-day plan, no account needed. Paid ($7.99/mo): a fresh 7-day plan delivered to your inbox every Sunday.

Tech: Next.js, Supabase, Claude API for generation, LemonSqueezy for billing, Vercel for hosting.

Happy to answer any questions about the build or the product.

---

## Priority Order

1. **Hacker News Show HN** — highest upside (real backlinks, real users, real feedback)
2. **Ben's Bites** — AI newsletter with highly relevant audience
3. **Future Tools + ToolPilot.ai** — AI discovery sites, quick wins for backlinks
4. **SaaSHub** — high DA, good for SEO
5. **There's An App For That** — easy submit, decent traffic
6. **Capterra/GetApp** — more work but high-authority backlinks
7. **G2 + SaaSWorthy** — review-driven, better once you have a few users to ask
