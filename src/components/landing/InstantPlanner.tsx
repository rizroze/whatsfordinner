...
export function InstantPlanner({ isSignedIn }: { isSignedIn?: boolean }) {
  ...
  return (
    <div id="instant-planner" className="mt-8 sm:mt-10 max-w-2xl mx-auto text-left scroll-mt-24">
      <div className="bg-white rounded-3xl border border-stone-100 shadow-md p-5 sm:p-7">
        {/* Diet tiles */}
        <p className="text-xs font-medium text-stone-500 mb-2.5">{t("instant.dietLabel")}</p>
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {DIETS.map((d) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 rounded-2xl border px-2 py-2.5 transition-all duration-200 ${
                diet === d.value
        ...
        <div className="mt-5 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
          <div className="flex items-center gap-2">
            <span className="text-sm text-stone-600">{t("instant.caloriesLabel")}</span>
            <input
              type="number"
              inputMode="numeric"
              min={1000}
              ...
              className="w-20 rounded-xl border border-stone-200 px-2 py-1.5 text-center text-sm font-semibold text-stone-800 focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100 transition-all duration-200"
            />
            <span className="text-sm text-stone-600">{t("instant.caloriesUnit")}</span>
          </div>
        ...
        {!plan && !loading && (
          <p className="mt-2.5 text-center text-[11px] text-stone-400">{t("instant.free")}</p>
        )}
        {error && (
          <p className="mt-2.5 text-center text-xs text-red-500">{t("instant.error")}</p>
        )}
        ...
        {plan && (
          <div className="mt-6 space-y-2">
            {plan.meals.map((m) => (
              <div
                key={m.slot}
                className={`flex items-center gap-3 rounded-2xl border border-stone-100 bg-[#FFFBF5] px-3.5 py-3 transition-opacity duration-200 ${
                  swappingSlot === m.slot ? "opacity-40" : "opacity-100"
                }`}
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white border border-stone-100 text-lg">
                  {m.emoji}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${SLOT_COLORS[m.slot]}`}>
                      {t(`mealTypes.${m.slot}`)}
                    </span>
                    {m.servings > 1 && (
                  ...
                  <Link
                    href={m.url}
                    className="mt-0.5 block truncate text-sm font-semibold text-stone-800 transition-colors hover:text-orange-600"
                ...
                <button
                  ...
                  className="shrink-0 rounded-full border border-stone-200 bg-white p-2 text-stone-400 transition-all duration-200 hover:border-orange-300 hover:text-orange-500 hover:shadow-sm active:scale-95 disabled:opacity-50"
                >
                  <svg
                    width="14"
                    height="14"
            ...
            {/* Total vs target */}
            <div className="flex items-center justify-between rounded-2xl bg-stone-50 px-4 py-2.5">
              <span className="text-xs font-medium text-stone-500">{t("instant.totalLabel")}</span>
      ...
      {plan && (
        <div className="mt-4 rounded-3xl border border-orange-200 bg-orange-50 p-5 sm:p-6 text-center">
          <p className="text-base sm:text-lg font-bold text-stone-900">{t("instant.upsellTitle")}</p>
          <p className="mx-auto mt-1.5 max-w-md text-xs sm:text-sm leading-relaxed text-stone-600">
            {t("instant.upsellBody")}
          </p>
          <a
            href={isSignedIn ? "/dashboard" : "#pricing"}
            onClick={() => track("instant_plan_upsell_click", { signedIn: !!isSignedIn })}
            className="mt-4 inline-flex items-center justify-center gap-2 rounded-full bg-orange-500 px-8 py-3 text-sm font-semibold text-white shadow-md transition-all duration-200 hover:bg-orange-600 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0"
          >
          ...
          {!isSignedIn && (
            <p className="mt-2 text-[11px] text-stone-400">{t("instant.upsellNote")}</p>
          )}
        </div>
      )}
    </div>
